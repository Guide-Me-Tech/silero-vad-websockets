...
class SileroVAD:
    ...
    def memory_vad_check(self, raw_audio: bytearray) -> Tuple[bool, List[Segment]]:
        ...
        audio_float32 = np.frombuffer(raw_audio_copy, dtype=np.int16).astype(np.float32) / 32768.0
...
class Client:
    ...
    def __init__(self, websocket, hub: Hub, vad: SileroVAD):
    ...
    async def process_audio(self):
        ...
        # Run VAD detection
        try:
            speech, segments = self.vad.memory_vad_check(self.buffer)
...
async def serve_websocket(websocket):
    ...
    # Create client
    client = Client(websocket, hub, vad)
